                ...
                <div class="p-6">
                    <form id="dashboard-filter" action="{{ route('admin.dashboard') }}" method="GET"
                            ...
                            <select id="kelas_id" name="kelas_id"
                                ...
                                @foreach ($allKelas as $kelas)
                                    <option value="{{ $kelas->id }}"
                                        {{ isset($selectedKelasId) && $selectedKelasId == $kelas->id ? 'selected' : '' }}>
                                        {{ $kelas->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex items-end">
                            <button type="submit"
                                class="px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:outline-none focus:border-indigo-700 focus:ring focus:ring-indigo-200 active:bg-indigo-700 transition mr-2">
                                Filter
                            </button>
                            <button type="button" id="refresh-data"
                                class="px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 focus:outline-none focus:border-green-700 focus:ring focus:ring-green-200 active:bg-green-700 transition mr-2">
                                <svg class="h-4 w-4 text-white inline-block" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                            ...
                            <a href="{{ route('admin.dashboard.export') }}?start_date={{ isset($startDate) ? $startDate->format('Y-m-d') : '' }}&end_date={{ isset($endDate) ? $endDate->format('Y-m-d') : '' }}&kelas_id={{ $selectedKelasId ?? '' }}"
                                class="px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 focus:outline-none focus:border-yellow-700 focus:ring focus:ring-yellow-200 active:bg-yellow-700 transition">
                                <svg class="h-4 w-4 text-white inline-block" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                <div id="total-siswa" class="text-3xl font-semibold text-gray-700 dark:text-gray-200">
                                    {{ $totalSiswa }}</div>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                <div id="total-guru" class="text-3xl font-semibold text-gray-700 dark:text-gray-200">
                                    {{ $totalGuru }}</div>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-5">
                                <div id="total-kelas" class="text-3xl font-semibold text-gray-700 dark:text-gray-200">
                                    {{ $totalKelas }}</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                    <div class="p-6 pb-20">
                        ...
                        <div id="attendance-trend-chart" class="h-80"></div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div id="attendance-by-class-chart" class="h-80"></div>
                    </div>
                </div>
            </div>
                ...
                <div class="p-6">
                        ...
                        <table id="recent-attendance-table" class="min-w-full bg-white dark:bg-gray-800">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-600" id="recent-attendance-body">
                                ...
                                @if (count($recentAbsensi) == 0)
                                    <tr>
                                        <td colspan="3"
                                            class="py-3 px-4 text-sm text-gray-500 dark:text-gray-300 text-center">
                    ...
                    <div class="mt-4 text-right">
                        <a href="{{ route('admin.absensi') }}"
                            class="text-indigo-600 dark:text-indigo-400 hover:underline">Lihat Semua</a>
                    </div>
                </div>
            </div>
