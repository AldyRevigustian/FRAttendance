        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('guru.absensi_store', ['kelas_terpilih' => session('kelas_aktif')]) }}" method="POST">
                        @csrf
                        <div class="mb-6">
                            <label for="siswa_id" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Nama Siswa</label>
                            <select name="siswa_id" id="siswa_id"
                                ...
                                @foreach ($siswas as $siswa)
                                    <option value="{{ $siswa->id }}" data-kelas-id="{{ $siswa->kelas_id }}" data-kelas-nama="{{ $siswa->kelas->nama ?? '' }}">
                                        {{ $siswa->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-6">
                            <label for="kelas_nama" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Kelas</label>
