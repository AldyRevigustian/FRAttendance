...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Students with face recognition training status
        $trainedSiswa = Siswa::where('is_trained', true)->count();
        $untrainedSiswa = Siswa::where('is_trained', false)->count();
        
        // Recent attendance records with optional class filter
        ...
        // Attendance by class in date range
        ...
        // Attendance trends for selected date range
        $absensiTrend = [];
        $dateRange = $endDate->diffInDays($startDate) + 1;
        $dateRange = min($dateRange, 30); // Limit to 30 days max for better visualization
        ...
        // Gender distribution
        $genderDistribution = [
            'laki-laki' => Siswa::where('jenis_kelamin', 'Laki-laki')->count(),
        ...
        if ($request->ajax()) {
            return response()->json([
                ...
                'trainedSiswa' => $trainedSiswa,
                'untrainedSiswa' => $untrainedSiswa,
    ...
    public function exportAbsensi(Request $request)
    {
        ...
        // Return CSV download response (in real implementation, you'd use a package like maatwebsite/excel)
